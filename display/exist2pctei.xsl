...
<xsl:template match="/TEI.2/teiHeader">
    <xsl:copy-of select="document('../cemHeader.xml')/teiHeader"/>
</xsl:template>
...
<xsl:template match="exist:request">
  ...
  <table rend="rules">
    ...
    <xsl:variable name="S">
     <xsl:value-of 
select="substring-before(substring-after(@exist:source,'/db/cem/'),'.xml')"/>
   </xsl:variable>
      <row>
        <cell>
          <xref url="{$S}.html">Stone <xsl:value-of select="$S"/></xref>
      </cell>
      <xsl:choose>
